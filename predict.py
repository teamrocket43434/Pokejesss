...
class Prediction:
    ...
    def generate_labels_file_from_save_path(self):
        if not os.path.exists(self.save_path): raise FileNotFoundError(f"SAVE_PATH not found: {self.save_path}")
        ...
        with open(self.labels_path, "w", encoding="utf-8") as f: json.dump(class_names, f, indent=2)
        return class_names

    def load_class_names(self):
        if not os.path.exists(self.labels_path): return self.generate_labels_file_from_save_path()
        with open(self.labels_path, "r", encoding="utf-8") as f:
            data = json.load(f)
            if isinstance(data, dict): return [data[k] for k in sorted(data, key=lambda x: int(x))]
            if isinstance(data, list): return data
            raise ValueError("labels_v2.json must be a list or dict")

    def preprocess_image_from_url(self, url):
        arr = np.frombuffer(requests.get(url, timeout=5).content, np.uint8)
        image = Image.open(io.BytesIO(arr)).convert("RGB").resize((224,224), Image.BILINEAR)
...
def main():
    try: predictor = Prediction()
    except Exception as e: return print(f"Initialization error: {e}")
    while True:
        url = input("Enter Pokémon image URL (or 'q' to quit): ").strip()
        if url.lower() == "q": break
        try: print("Predicted Pokémon:", *predictor.predict(url))
